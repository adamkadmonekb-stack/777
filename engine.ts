...
if (this.state.activeVehicle) {
  ...
  private computePrompt() {
    ...
    if (best) {
      switch (best.kind) {
        case 'door_shop': { const sh = SHOPS.find(s => s.id === best!.data); this.prompt = `${sh?.name ?? 'Магазин'} — посмотреть витрину`; break; }
        ...
        case 'pond': this.prompt = this.hasItem('rod') ? 'Рыбачить [мини-игра]' : 'Пруд. Нужна удочка'; break;
...
this.updateAchievement('apartment_cook_15', s.cookCount);
  ...
  interact() {
    ...
    switch (o.kind) {
      case 'door_shop': this.openModal({ kind: 'shop', id: o.data ?? 'pyaterochka' }); break; // витрина без входа внутрь
      ...
      case 'door_apart':
        ...
        break;
      case 'pond':
        if (this.hasItem('rod')) this.openModal({ kind: 'minigame', game: 'fish', data: {} });
        else this.toast('Рыбак Михалыч: «Без удочки никак. Ищи в Строймаркете»', 'info');
        break;
...
this.updateAchievement('work_30days', 1);
  ...
  nearbyPois(): { name: string; dist: number }[] {
    ...
    for (const o of this.world.objects) {
      ...
      else if (o.kind === 'pond') pois.push({ name: 'Пруд (рыбалка)', x: o.x, y: o.y });
    }
    return pois
