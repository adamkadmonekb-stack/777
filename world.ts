...
export type ObjKind =
  ...
  | 'door_soup' | 'door_granny' | 'door_realtor' | 'door_apart' | 'pond'
...
const pondX = 170, pondY = 115;               // Новый пруд (правый нижний угол)
...
const safeBld = (x: number, y: number, w: number, h: number, name: string, wall: string, roof: string, kind: string, data?: string, sign?: string) => {
  ...
  // ---- парк ----
  // ПРУД ПЕРЕНЕСЁН в правый нижний угол (координаты 170, 115)
  const pondCx = 170, pondCy = 115, pondRx = 10, pondRy = 6;
  for (let j = 105; j < 125; j++) {
    for (let i = 155; i < 185; i++) {
      const d = ((i - pondCx) / pondRx) ** 2 + ((j - pondCy) / pondRy) ** 2;
      if (d <= 1) set(i, j, 5);      // Вода
      else if (d <= 1.5) set(i, j, 6); // Песок/берег
    }
  }
  objects.push({ id: 'pond', kind: 'pond', x: px(170), y: px(119) });
  
  // ПАРКОВКА НАД ПРУДОМ (асфальтированная, 8 мест)
  ...
  // Припаркованные машины (для атмосферы)
   // objects.push({ id: 'parked_car_1', kind: 'bench', x: px(parkingPondX + 2), y: px(parkingPondY + 7), data: 'car_gray' });
   // objects.push({ id: 'parked_car_2', kind: 'bench', x: px(parkingPondX + 12), y: px(parkingPondY + 7), data: 'car_black' });
  //  objects.push({ id: 'parked_car_3', kind: 'bench', x: px(parkingPondX + 27), y: px(parkingPondY + 7), data: 'car_white' });
  //  objects.push({ id: 'parking_pond', kind: 'bench', x: px(parkingPondX + 20), y: px(parkingPondY + 7), data: 'parking_pond' });
...
// objects.push({ id: 'busstop2', kind: 'busstop', x: px(74), y: px(39.6) + 6 }); 
  ...
  objects.push({ id: 'azs_entrance', kind: 'worker', x: px(azsX + 3), y: px(azsY + 4) - 6, data: 'azs' });
...
objects.push({ id: 'player_truck', kind: 'vehicle' as any, x: px(parkingX + 9), y: px(parkingY + 4), data: 'truck_gaz' });
  ...
  npcs.push({ id: 'fisher', kind: 'fisher', x: px(pondX - 12), y: px(pondY + 8), hx: px(pondX - 12), hy: px(pondY + 8), r: 12, tx: px(pondX - 12), ty: px(pondY + 8), speed: 0, seed: 21, anim: 0, moving: false });
  for (let i = 0; i < 3; i++) {
